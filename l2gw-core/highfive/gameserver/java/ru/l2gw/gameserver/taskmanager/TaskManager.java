...
import ru.l2gw.gameserver.taskmanager.tasks.TaskHuningBonus;
import ru.l2gw.gameserver.taskmanager.tasks.TaskRecom;
...
import static ru.l2gw.gameserver.taskmanager.TaskTypes.*;
...
{
	static final Log _log = LogFactory.getLog(TaskManager.class.getName());
	...
	static final String[] SQL_STATEMENTS = {
			...
			"TRUNCATE TABLE global_tasks"};

	private final FastMap<Integer, Task> _tasks = new FastMap<Integer, Task>();
	final FastList<ExecutedTask> _currentTasks = new FastList<ExecutedTask>();
	...
	public class ExecutedTask implements Runnable
	{
		...
		public ExecutedTask(Task task, TaskTypes type, ResultSet rset) throws SQLException
		{
			...
			//System.out.println("TASK EXECUTER: ADDED TASK "+task.getName());
		}
	...
	private void initializate()
	{
		...
		registerTask(new TaskHuningBonus());
	}
	...
	public void registerTask(Task task)
	{
		...
		if(!_tasks.containsKey(key))
		{
			...
			//System.out.println("_TASKS: puted "+task.getName());
	...
	public void startAllTasks()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				int key = rset.getString("task").hashCode();
				Task task = _tasks.get(key);

				//System.out.println("Task in DB: "+);
		...
		{
			_log.warn("error while loading Global Task table " + e);
			e.printStackTrace();
		}
		finally
		{
	...
	private static void cleanUp()
	{
		...
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement(SQL_STATEMENTS[4]);
			rset = statement.executeQuery();
		} catch (Exception e) {}
	}
