...
public class L2RaceManagerInstance extends L2NpcInstance
{
	...
	public L2RaceManagerInstance(int objectId, L2NpcTemplate template, long bossIndex, long p1, long p2, long p3)
	{
	...
	public void showMonsterInfo(L2Player player)
	{
		...
		String filename, search;
		NpcHtmlMessage html = new NpcHtmlMessage(_objectId);
		filename = getHtmlPath(npcId, 6, player.getKarma());
		html.setFile(filename);
		for(int i = 0; i < 8; i++)
		{
			int n = i + 1;
	...
	public void showBuyTicket(L2Player player, int val)
	{
		...
		if(val < 10)
		{
			filename = getHtmlPath(npcId, 2, player.getKarma());
			html.setFile(filename);
			for(int i = 0; i < 8; i++)
			{
				int n = i + 1;
		...
		else
		{
			if(player.getRace(0) == 0 || player.getRace(1) == 0)
				return;
			if(player.reduceAdena("MonsterRace", cost[player.getRace(1) - 1], this, true))
			{
				...
				player.getInventory().addItem("MonsterRace", item, player, this);
			}
		}
		html.replace("1race", String.valueOf(_raceNumber));
		player.sendPacket(html);
