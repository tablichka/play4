...
import static ru.l2gw.gameserver.model.base.ClassLevel.First;
import static ru.l2gw.gameserver.model.base.ClassLevel.Fourth;
import static ru.l2gw.gameserver.model.base.ClassLevel.Second;
import static ru.l2gw.gameserver.model.base.ClassLevel.Third;
import static ru.l2gw.gameserver.model.base.ClassType.Fighter;
import static ru.l2gw.gameserver.model.base.ClassType.Mystic;
import static ru.l2gw.gameserver.model.base.ClassType.Priest;
import static ru.l2gw.gameserver.model.base.Race.darkelf;
import static ru.l2gw.gameserver.model.base.Race.dwarf;
import static ru.l2gw.gameserver.model.base.Race.elf;
import static ru.l2gw.gameserver.model.base.Race.human;
import static ru.l2gw.gameserver.model.base.Race.kamael;
import static ru.l2gw.gameserver.model.base.Race.orc;
...
public enum PlayerClass
{
	...
	Necromancer(human, Mystic, Third), //13	//mod--
	...
	humanShaman(human, Mystic, Second), //58
	humanOverlord(human, Mystic, Third), //59
	humanDominator(human, Mystic, Fourth), //60
	orcWizard(orc, Mystic, Second), //61
	orcNecromancer(orc, Mystic, Third), //62
	orcSoultaker(orc, Mystic, Fourth), //63
	darkKnight(darkelf, Fighter, Second), //64
	dDarkAvenger(darkelf, Fighter, Third), //65
	dhellKnight(darkelf, Fighter, Fourth), //66
	dwarvenCleric(dwarf, Priest, Second), //67
	dwarvenbishop(dwarf, Priest, Third), //68
	dwarvencardinal(dwarf, Priest, Fourth), //69
	...
	private static final Set<PlayerClass> subclasseSet6 = EnumSet.of(ElvenElder, ShillienElder);
	private static final Set<PlayerClass> subclasseSet7 = EnumSet.of(Swordsinger, Bladedancer);

	public static final EnumMap<PlayerClass, Set<PlayerClass>> subclassSetMap = new EnumMap<PlayerClass, Set<PlayerClass>>(PlayerClass.class);
	...
	static
	{
		...
		subclassSetMap.put(ElvenElder, subclasseSet6);
		subclassSetMap.put(EvaSaint, subclasseSet6);
		subclassSetMap.put(ShillienElder, subclasseSet6);
		subclassSetMap.put(ShillienSaint, subclasseSet6);

		subclassSetMap.put(SwordMuse, subclasseSet7);
		subclassSetMap.put(Swordsinger, subclasseSet7);
		subclassSetMap.put(Bladedancer, subclasseSet7);
		subclassSetMap.put(SpectralDancer, subclasseSet7);

		subclassSetMap.put(Doombringer, EnumSet.of(Inspector));
		subclassSetMap.put(MaleSoulHound, EnumSet.of(Inspector));
		subclassSetMap.put(FemaleSoulHound, EnumSet.of(Inspector));
		subclassSetMap.put(Trickster, EnumSet.of(Inspector));
	...
	public final Set<PlayerClass> getAvailableSubclasses()
	{
		...
		if(_level == Third || _level == Fourth)
		{
			...
			//убираем левые сабклассы
			for(PlayerClass player_class : subclasses)
				if(player_class.ordinal() >= 58 && player_class.ordinal() <= 87)
					subclasses.remove(player_class);
		}
