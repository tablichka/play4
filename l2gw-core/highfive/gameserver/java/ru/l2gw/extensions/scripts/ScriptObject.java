...
{
	private static final Log _log = LogFactory.getLog(ScriptObject.class.getName());
	...
	public void setProperty(String s, Object o)
	{
		...
		try
		{
			...
			if(fld == null)
				_log.warn("Class " + getName() + " field " + s + " not found!");
			else if(!Modifier.isPublic(fld.getModifiers()))
				_log.warn("Class " + getName() + " field " + s + " is not public!");
			else
				fld.set(_instance, o);
		}
		catch(NoSuchFieldException e)
		{
			_log.warn("Class " + getName() + " field " + s + " not found or private!");
		}
		catch(Exception e)
		{
			e.printStackTrace();
	...
	public Object invokeMethod(Method m)
	{
		...
		{
			_log.warn("Class " + getName() + " method " + m + " return a error!");
			f.printStackTrace();
		}
		catch(Exception e)
		{
	...
	public Object invokeMethod(String s)
	{
		...
		try
		{
			...
			if(m == null)
				_log.warn("Class " + getName() + " method " + s + " not found!");
			else
				return m.invoke(Modifier.isStatic(m.getModifiers()) ? null : _instance);
		}
		...
		{
			_log.warn("Class " + getName() + " method " + s + " return a error!");
			f.printStackTrace();
		}
		catch(Exception e)
		{
