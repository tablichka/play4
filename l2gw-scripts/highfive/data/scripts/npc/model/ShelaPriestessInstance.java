...
/**
 * @author rage
 * @date 16.11.2010 15:43:22
 */
public class ShelaPriestessInstance extends CarlInstance
{
	public ShelaPriestessInstance(int objectId, L2NpcTemplate template, long bossIndex, long p1, long p2, long p3)
	{
	...
	public void showChatWindow(L2Player player, int val)
	{
		...
		if(val == 0)
		{
			...
			if(qs.getInt("t1") < 0)
			{
				...
				{
					qs.deleteRadar(-56736, -113680, -672, 2);
					...
					showPage(player, player.isMageClass() ? "shela_priestess_of_fire001.htm" : "carl001.htm");
				}
				else
					showPage(player, "carl006.htm");
			}
			else if(qs.getInt("t1") >= 0 && qs.getInt("t1") <= 2 && !qs.haveQuestItems(6353))
				showPage(player, player.isMageClass() ? "shela_priestess_of_fire002.htm" : "carl002.htm");
			else if(qs.getInt("t1") >= 0 && qs.getInt("t1") <= 2 && qs.haveQuestItems(6353))
			{
				qs.takeItems(6353, -1);
				qs.set("t1", 3);
				...
				qs.set("t", i0 | 4);
				if(!qs.haveQuestItems(5789))
					qs.giveItems(5789, 200);

				qs.playTutorialVoice("tutorial_voice_026", 1000);
				showPage(player, player.isMageClass() ? "shela_priestess_of_fire003m.htm" : "shela_priestess_of_fire003f.htm");
			}
			else if(qs.getInt("t1") == 3)
				showPage(player, "shela_priestess_of_fire004.htm");
			else
				showPage(player, "carl005.htm");
		}
	}
