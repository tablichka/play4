...
/**
 * @author rage
 * @date 16.11.2010 15:39:31
 */
public class MotherAInstance extends CarlInstance
{
	public MotherAInstance(int objectId, L2NpcTemplate template, long bossIndex, long p1, long p2, long p3)
	{
	...
	public void showChatWindow(L2Player player, int val)
	{
		...
		if(val == 0)
		{
			...
			if(qs.getInt("t1") < 0)
			{
				...
				{
					qs.deleteRadar(46112, 41200, -3504, 2);
			...
			else if(qs.getInt("t1") >= 0 && qs.getInt("t1") <= 2 && qs.haveQuestItems(6353))
			{
				...
				showPage(player, player.isMageClass() ? "mother_temp_a003m.htm" : "mother_temp_a003f.htm");
			}
			else if(qs.getInt("t1") == 3)
				showPage(player, "mother_temp_a004.htm");
			else
				showPage(player, "carl005.htm");
		}
	}
		...
		if(player != null && !player.isDead())
		{
			...
			if(qs != null)
			{
				if(qs.getInt("t1") == 0)
				{
					...
					qs.playTutorialVoice("tutorial_voice_010c", 0);
			}
		}
	}
}
