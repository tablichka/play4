...
public class FantasyIsle extends Functions implements ScriptFile
{
	...
	private static HashMap<String, Walk> WALKS = new HashMap<String, Walk>();
	static
	{
	...
	private static HashMap<String, Talk> TALKS = new HashMap<String, Talk>();
	static
	{
	...
	public void onLoad()
	{
		_log.info("Loaded Service: Fantasy Isle");

		_startTask = ThreadPoolManager.getInstance().scheduleAiAtFixedRate(new StartTask(), 60000, 60000);
	}
	...
	private static void startQuestTimer(String event, int time, L2NpcInstance temp_npc)
	{
		HashMap<String, Object> variables = new HashMap<String, Object>();
		variables.put("npc", temp_npc);
		executeTask(temp_npc, "services.FantasyIsle", "start", new Object[] { event }, variables, time);
	}
	...
	{
		L2NpcInstance temp_npc = npc;
		if(event.equals("Start"))
		{
			L2NpcInstance mc = addSpawn(MC, -56698, -56430, -2008, 32768);
			Functions.npcSay(mc, Say2C.ALL, TEXT[0]);
		...
		else if(TALKS.containsKey(event) && temp_npc != null)
		{
			...
			Functions.npcSay(temp_npc, Say2C.ALL, text);
		...
		{
			Functions.npcSay(temp_npc, Say2C.ALL, TEXT[6]);
			temp_npc.moveToLocation(new Location(-56511, -56647, -2008, 36863), 0, true);
			temp_npc.broadcastPacket(new PlaySound(1, "NS22_F", 0, 0, new Location(0, 0, 0)));
		...
		else if(event.equals("7") && temp_npc != null)
		{
			...
			{
				Functions.npcSay(temp_npc, Say2C.ALL, TEXT[7]);
		...
		else if(event.equals("10") && temp_npc != null)
		{
			...
			temp_npc.broadcastPacket(new PlaySound(1, "TP05_F", 0, 0, new Location(0, 0, 0)));
		...
		else if(event.equals("11") && temp_npc != null)
		{
			if(temp_npc.getNpcId() == MC)
			{
				Functions.npcSay(temp_npc, Say2C.ALL, TEXT[10]);
		...
		{
			Functions.npcSay(temp_npc, Say2C.ALL, TEXT[15]);
		...
		{
			Functions.npcSay(temp_npc, Say2C.ALL, "We love you.");
