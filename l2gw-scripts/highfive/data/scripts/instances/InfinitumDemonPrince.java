...
/**
 * @author rage
 * @date 27.10.2010 16:37:51
 */
public class InfinitumDemonPrince extends Instance
{
	private static final Location _6thFloor = new Location(-19017, 277056, -8256);
	private boolean _success = false;

	public InfinitumDemonPrince(InstanceTemplate template, int rId)
	{
		super(template, rId);
	}
	...
	public void stopInstance()
	{
		...
		for(Integer objectId : _members)
		{
			...
			if(player != null && !player.isDeleting() && !player.isTeleporting() && _template.getZone().isInsideZone(player))
			{
				...
				if(_success)
					player.teleToLocation(_6thFloor, 0);
				else
