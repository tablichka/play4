package quests._373_SupplierOfReagents;
...
public class _373_SupplierOfReagents extends Quest
{
	...
	private static final int PlatinumTribeShama = 20828;
	...
	private static final int[][] DROPLIST_COND = {
			{1, 0, Crendion, 0, ROTTEN_BONE, 0, 62, 1},
			{1, 0, Crendion, 0, QUICKSILVER, 0, 100, 1},
			{1, 0, HallatesMaid, 0, REAGENT_POUCH1, 0, 66, 1},
			{1, 0, HallatesMaid, 0, VOLCANIC_ASH, 0, 84, 1},
			{1, 0, HallatesGuardian, 0, DEMONS_BLOOD, 0, 73, 1},
			{1, 0, HallatesGuardian, 0, MOONSTONE_SHARD, 0, 83, 1},
			{1, 0, PlatinumTribeShama, 0, REAGENT_POUCH2, 0, 68, 1},
			{1, 0, PlatinumTribeShama, 0, QUICKSILVER, 0, 100, 1},
			{1, 0, PlatinumGuardianShaman, 0, REAGENT_BOX, 0, 44, 1},
			{1, 0, LavaWyrm, 0, WYRMS_BLOOD, 0, 50, 1},
			{1, 0, LavaWyrm, 0, LAVA_STONE, 0, 75, 1},
			{1, 0, HamesOrcShaman, 0, REAGENT_POUCH3, 0, 47, 1}};
	...
	{
		super(373, "_373_SupplierOfReagents", "Supplier Of Reagents"); // Party true
		addStartNpc(WESLEY);
		addTalkId(URN);
	...
	public String render_urn(QuestState st, String[] page)
	{
		...
		if(page == null)
		{
			...
			String temp = (String) st.get("temp");
			String r_ingr = "";
			if(ingr != 0)
			{
	...
	public String onEvent(String event, QuestState st)
	{
		...
		else if(event.startsWith("x"))
		{
			...
			if(st.getQuestItemsCount(item) >= qty)
			{
				...
				if(dst.equals("C"))
				{
					st.set("c_qty", String.valueOf(qty));
					st.set("catalyst", String.valueOf(item));
				}
				else
				{
					st.set("i_qty", String.valueOf(qty));
					st.set("ingredient", String.valueOf(item));
				}
			}
			else
			...
			if(st.getInt("mixing") > 0)
			{
				...
				if(temp != 0)
				{
					...
					if(item > 0)
					{
						...
						if(item == MIMIRS_ELIXIR)
						{
							QuestState mimirs = st.getPlayer().getQuestState("_235_MimirsElixir");
							if(mimirs != null)
							{
								chance = 100;
								qty = 1;
	...
	public void onKill(L2NpcInstance npc, QuestState st)
	{
		...
		{
			if(npcId == aDROPLIST_COND[2])
			{
				// We must have fixed chance here, so other items from mob can be given to player
				if(Rnd.chance(aDROPLIST_COND[6]) && st.rollAndGive(aDROPLIST_COND[4], aDROPLIST_COND[7], 100))
