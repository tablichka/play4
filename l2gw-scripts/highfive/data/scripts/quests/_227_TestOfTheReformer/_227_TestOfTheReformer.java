...
public class _227_TestOfTheReformer extends Quest
{
	...
	public final int[][] DROPLIST_COND = {
			{18, 0, SilentHorror, 0, BoneFragment1, 1, 70, 1},
			{18, 0, SkeletonLord, 0, BoneFragment2, 1, 70, 1},
			{18, 0, SkeletonMarksman, 0, BoneFragment3, 1, 70, 1},
			{18, 0, MiserySkeleton, 0, BoneFragment4, 1, 70, 1},
			{18, 0, SkeletonArcher, 0, BoneFragment5, 1, 70, 1}};

	private static boolean QuestProf = true;

	public _227_TestOfTheReformer()
	{
		super(227, "_227_TestOfTheReformer", "Test Of The Reformer");
		addStartNpc(Pupina);
		addTalkId(Sla);
		addTalkId(Katari);
	...
	public String onEvent(String event, QuestState st)
	{
		...
		else if(event.equalsIgnoreCase("30669-03.htm"))
		{
			if(L2ObjectsStorage.getByNpcId(CrimsonWerewolf) == null)
			{
				st.set("cond", "12");
				st.setState(STARTED);
				st.getPcSpawn().addSpawn(CrimsonWerewolf);
		...
		else if(event.equalsIgnoreCase("30670-03.htm"))
		{
			if(L2ObjectsStorage.getByNpcId(KrudelLizardman) == null)
			{
				st.set("cond", "15");
				st.setState(STARTED);
				st.getPcSpawn().addSpawn(KrudelLizardman);
		...
		else if(npcId == Katari)
		{
			if(cond == 5 || cond == 6)
			{
				...
				if(NPC == null && Mob == null)
				{
					...
					st.set("cond", "6");
					st.setState(STARTED);
					st.getPcSpawn().addSpawn(OlMahumPilgrimNPC);
					st.getPcSpawn().addSpawn(OlMahumInspector);
			...
			else if(cond == 8)
			{
				if(L2ObjectsStorage.getByNpcId(OlMahumBetrayer) == null)
				{
					...
					st.getPcSpawn().addSpawn(OlMahumBetrayer);
	...
	public void onKill(L2NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == NamelessRevenant && (cond == 1 || cond == 2))
		{
			...
			else if(L2ObjectsStorage.getByNpcId(Aruraune) == null)
			{
				...
				st.set("cond", "2");
				st.setState(STARTED);
				st.getPcSpawn().addSpawn(Aruraune);
				st.startQuestTimer("Wait1", 300000);
			}
			else
			{
				if(st.getQuestTimer("Wait1") == null)
					st.startQuestTimer("Wait1", 300000);
			}
		}
