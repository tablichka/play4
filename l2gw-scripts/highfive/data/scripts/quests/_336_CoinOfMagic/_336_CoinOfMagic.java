...
public class _336_CoinOfMagic extends Quest
{
	...
	private static final int[] BASIC_COINS = {BLOOD_MEDUSA, GOLD_WYVERN, SILVER_UNICORN};
	...
	{
		super(336, "_336_CoinOfMagic", "Coin Of Magic"); // Party true
		addStartNpc(SORINT);
	...
	public void onKill(L2NpcInstance npc, L2Player killer)
	{
		...
		if(npcId == HaritLizardmanMatriarch || npcId == HaritLizardmanShaman)
		{
			...
			if(st != null)
			{
				if(st.rollAndGiveLimited(KALDIS_COIN, 1, 10 * npc.getTemplate().hp_mod, 1))
				{
					st.set("cond", "3");
			...
			for(int u : UNKNOWN)
				if(u == npcId)
				{
					...
					if(st != null && st.getCond() > 3)
					{
						int grade = st.getInt("grade");
						int chance = npc.getLevel() - grade * 3 - 20;
						st.rollAndGive(BASIC_COINS[Rnd.get(BASIC_COINS.length)], 1, chance * npc.getTemplate().hp_mod);
						return;
					}
				}
		}
