package quests._384_WarehouseKeepersPastime;
...
public class _384_WarehouseKeepersPastime extends Quest
{
	...
	private final static FastMap<Integer, Integer> Medal_Chances = new FastMap<Integer, Integer>();
	private final static FastMap<Integer, Bingo> bingos = new FastMap<Integer, Bingo>();

	private final static int[][] Rewards_Win = {{16, 1888, 1}, //Synthetic Cokes
			{32, 1887, 1}, //Varnish of Purity
			{50, 1894, 1}, //Crafted Leather
			{80, 952, 1}, //Scroll: Enchant Armor (C)
			{89, 1890, 1}, //Mithril Alloy
			{98, 1893, 1}, //Oriharukon
			{100, 951, 1} //Scroll: Enchant Weapon (C)
	};
	private final static int[][] Rewards_Win_Big = {{50, 883, 1}, //Aquastone Ring
			{80, 951, 1}, //Scroll: Enchant Weapon (C)
			{98, 852, 1}, //Moonstone Earring
			{100, 401, 1} //Drake Leather Armor
	};
	private final static int[][] Rewards_Lose = {{50, 4041, 1}, //Mold Hardener
			{80, 952, 1}, //Scroll: Enchant Armor (C)
			{98, 1892, 1}, //Blacksmith's Frame
			{100, 917, 1} //Necklace of Mermaid
	};
	private final static int[][] Rewards_Lose_Big = {{50, 951, 1}, //Scroll: Enchant Weapon (C)
			{80, 500, 1}, //Great Helmet
			{98, 2437, 2}, //Drake Leather Boots
			{100, 135, 1} //Samurai Longsword
	};

	public _384_WarehouseKeepersPastime()
	{
		super(384, "_384_WarehouseKeepersPastime", "Warehouse Keeper’s Pastime");
		addStartNpc(Cliff);
		addTalkId(Baxt);
	...
	public String onEvent(String event, QuestState st)
	{
		...
		if(event.equalsIgnoreCase("30182-05.htm") && st.isCreated())
		{
			st.set("cond", "1");
			st.setState(STARTED);
			st.playSound(SOUND_ACCEPT);
		}
		else if((event.equalsIgnoreCase("30182-08.htm") || event.equalsIgnoreCase("30685-08.htm")) && st.isStarted())
		{
			st.playSound(SOUND_FINISH);
			st.exitCurrentQuest(true);
		}
		else if(event.contains("-game") && st.isStarted())
		{
	...
	public void onKill(L2NpcInstance npc, QuestState st)
	{
		...
		if(chance != null && st.rollAndGive(Warehouse_Keepers_Medal, 1, chance))
			st.playSound(st.getQuestItemsCount(Warehouse_Keepers_Medal) == 10 ? SOUND_MIDDLE : SOUND_ITEMGET);
