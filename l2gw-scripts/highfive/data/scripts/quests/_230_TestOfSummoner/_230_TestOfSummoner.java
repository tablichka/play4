package quests._230_TestOfSummoner;
...
public class _230_TestOfSummoner extends Quest
{
	...
	static int[][] SUMMONERS = {{30635, ALMORS_ARCANA_ID, CRYSTAL_OF_VICTORY1_ID}, // Almors
			{30636, CAMONIELL_ARCANA_ID, CRYSTAL_OF_VICTORY2_ID},// Camoniell
			{30637, BELTHUS_ARCANA_ID, CRYSTAL_OF_VICTORY3_ID}, // Belthus
			{30638, BASILLIA_ARCANA_ID, CRYSTAL_OF_VICTORY4_ID}, // Basilla
			{30639, CELESTIEL_ARCANA_ID, CRYSTAL_OF_VICTORY5_ID},// Celestiel
			{30640, BRYNTHEA_ARCANA_ID, CRYSTAL_OF_VICTORY6_ID} // Brynthea
	};
	...
	static HashMap<Integer, Integer[]> DROPLIST_LARA = new HashMap<Integer, Integer[]>();
	...
	static String[] STATS = {"cond", "step", "Lara_Part", "Arcanas", "Belthus", "Brynthea", "Celestiel", "Camoniell", "Basilla", "Almors"};

	static int[][] LISTS = {{}, // zero element filler
			{LARS_LIST1_ID, SAC_OF_REDSPORES_ID, LETOLIZARDMAN_AMULET_ID}, // List 1
			{LARS_LIST2_ID, KARULBUGBEAR_TOTEM_ID, SHARDS_OF_MANASHEN_ID}, // List 2
			{LARS_LIST3_ID, CRIMSON_BLOODSTONE_ID, BREKAORC_TOTEM_ID}, // List 3
			{LARS_LIST4_ID, TUSK_OF_WINDSUS_ID, TALONS_OF_TYRANT_ID}, // List 4
			{LARS_LIST5_ID, WINGS_OF_DRONEANT_ID, FANGS_OF_WYRM_ID} // List 5
	};

	public _230_TestOfSummoner()
	{
		super(230, "_230_TestOfSummoner", "Test Of Summoner");
	...
	public String onTalk(L2NpcInstance npc, QuestState st)
	{
		...
		else if(st.isStarted())
		{
			...
			int step = st.getInt("step"); // stats as short vars if( the player has state <Progress>
			if(npcId == 30634)
			{ // Start&&End Npc Galatea related stuff
				...
				{ // step 2 means already talkd with lara
					...
					{ // finished all battles... the player is able to earn the marks
						...
						if(!st.getPlayer().getVarB("q230"))
						{
							st.addExpAndSp(832247, 57110);
							st.rollAndGive(57, 150480, 100);
							st.getPlayer().setVar("q230", "1");
						}
						st.playSound(SOUND_FINISH);
						st.unset("cond");
						st.exitCurrentQuest(true);
				...
				for(int[] i : SUMMONERS)
					if(i[0] == npcId)
					{
						...
						if(step > 1)
							...
							else if(SummonerStat == 2) // ready to fight... already take the mission to kill his pet
							...
							else if(SummonerStat == 4) // haha... your summon lose
								htmltext = str(npcId) + "-05.htm";
							else if(SummonerStat == 5) // hey.. shit cheater.. dont help your pet
	...
	static HashMap<Integer, Integer[]> DROPLIST_SUMMON = new HashMap<Integer, Integer[]>();
	...
	static HashMap<Integer, String> DROPLIST_SUMMON_VARS = new HashMap<Integer, String>();
	...
	@Override
	public String onDeath(L2NpcInstance killer, L2Character victim, QuestState st)
	...
	public void onKill(L2NpcInstance npc, QuestState st)
	{
		...
		{ // if a summon dies
			...
			if(st.getInt(var) == 3)
			{
				ConcurrentHashMap<Integer, AggroInfo> attackerList = npc.getAggroList();
				...
				if(attackerList.size() == 2 && st.getPlayer().getPet() != null && attackerList.containsKey(st.getPlayer().getPet().getObjectId()) && attackerList.get(st.getPlayer().getObjectId()).damage == 0)
				{
					st.set(var, "6");
