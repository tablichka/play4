package quests._235_MimirsElixir;
...
public class _235_MimirsElixir extends Quest
{
	...
	//Messages
	private final static String _default = "noquest";
	...
	{
		super(235, "_235_MimirsElixir", "Mimirs Elixir");
		...
		addQuestItem(TRUE_GOLD, SAGES_STONE, BLOOD_FIRE, MIMIRS_ELIXIR);
	}
	...
	public String onTalk(L2NpcInstance npc, QuestState st)
	{
		...
		if(npcId == LADD)
		{
			...
			{
				st.set("cond", "0");
				if(st.getPlayer().getLevel() < MINLEVEL || st.getPlayer().getRace().ordinal() == 5)
				{
					st.exitCurrentQuest(true);
					htmltext = "30721-01.htm"; //not qualified
				}
				else if(st.getPlayer().getQuestState("_234_FatesWhisper") != null && st.getPlayer().getQuestState("_234_FatesWhisper").isCompleted() || st.getQuestItemsCount(STAR_OF_DESTINY) > 0)
				{
					if(st.getQuestItemsCount(STAR_OF_DESTINY) > 0)
						htmltext = "30721-02.htm"; // Successful start: Bring me Pure silver from Reagents quest
					else
				...
				{
					st.exitCurrentQuest(true);
					htmltext = "30721-01a.htm"; //not qualified
				}
			}
			...
			else if(cond == 8)
			{
				...
				npc.altUseSkill(SkillTable.getInstance().getInfo(4339, 1), st.getPlayer());
	...
	public void onKill(L2NpcInstance npc, QuestState st)
	{
		int npcId = npc.getNpcId();
		int cond = st.getInt("cond");
		if(npcId == Chimera_Piece && cond == 3 && st.rollAndGiveLimited(SAGES_STONE, 1, chance, 1))
		{
			st.playSound(SOUND_MIDDLE);
			st.set("cond", str(cond + 1));
			st.setState(STARTED);
		}
		if(npcId == Bloody_Guardian && cond == 6 && st.rollAndGiveLimited(BLOOD_FIRE, 1, chance, 1))
		{
			st.playSound(SOUND_MIDDLE);
			st.set("cond", str(cond + 1));
			st.setState(STARTED);
