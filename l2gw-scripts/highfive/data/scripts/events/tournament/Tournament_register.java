...
public class Tournament_register extends Functions
{
	...
	public static void bypass_listOfTeams()
	{
		...
		out += "</table>";
		show(out, (L2Player) self);
	}

	public static void bypass_listOfTeamsForRegister()
	{
		L2Player player = (L2Player) self;
		Integer category = Tournament_data.getCategory((int) player.getLevel());
	...
	public static void bypass_teamInfo(String[] var)
	{
		L2Player player = (L2Player) self;
		if(var.length != 1)
		{
	...
	public static void bypass_createTeam()
	{
		if(!active)
		{
			show("Извините, но периуд регистрации окончен. Дождитесь окончания турнира.", (L2Player) self);
			return;
		}
		show("data/scripts/events/tournament/32130-1.html", (L2Player) self);
	}

	public synchronized static void bypass_createTeam(String[] var)
	{
		if(!active)
		{
			show("Извините, но периуд регистрации окончен. Дождитесь окончания турнира.", (L2Player) self);
		...
		if(!StringUtil.isMatchingRegexp(team_name, Config.CLAN_NAME_TEMPLATE))
		{
	...
	public synchronized static void bypass_register(String[] var)
	{
		if(!active)
		{
			show("Извините, но периуд регистрации окончен. Дождитесь окончания турнира.", (L2Player) self);
	...
	public synchronized static void bypass_deleteTeam()
	{
		if(!active)
		{
			show("Извините, но периуд регистрации окончен. Дождитесь окончания турнира.", (L2Player) self);
	...
	public synchronized static void bypass_deleteMemberByLeader(String[] var)
	{
		if(!active)
		{
			show("Извините, но периуд регистрации окончен. Дождитесь окончания турнира.", (L2Player) self);
	...
	public static void bypass_leave()
	{
		if(!active)
		{
			show("Извините, но периуд регистрации окончен. Дождитесь окончания турнира.", (L2Player) self);
