 ...
 * @date: 19.01.12 16:24
 */
public class RoyalRushWarriorPhysicalspecial1Hold extends Warrior
{
	public L2Skill PhysicalSpecial1 = SkillTable.getInstance().getInfo(262209537);
	public int WeaponID = 0;

	public RoyalRushWarriorPhysicalspecial1Hold(L2Character actor)
	{
		super(actor);
	}
	...
	protected void onEvtAttacked(L2Character attacker, int damage, L2Skill skill)
	{
		...
		if(attacker.isPlayer() || CategoryManager.isInCategory(12, attacker.getNpcId()))
		{
			...
			{
				if(Rnd.get(100) < 33 && _thisActor.getMostHated() == attacker && _thisActor.getLoc().distance3D(attacker.getLoc()) > 100 && _thisActor.i_ai0 == 0)
				{
					...
					{
						addUseSkillDesire(attacker, PhysicalSpecial1, 0, 0, 1000000);
					}
				}
				if(Rnd.get(100) < 33 && _thisActor.i_ai0 == 1)
				{
	...
	protected void onEvtClanAttacked(L2Character attacked_member, L2Character attacker, int damage)
	{
		...
		{
			if(Rnd.get(100) < 33 && _thisActor.getLoc().distance3D(attacker.getLoc()) > 100 && _thisActor.i_ai0 == 0)
			{
				if(PhysicalSpecial1.getMpConsume() < _thisActor.getCurrentMp() && PhysicalSpecial1.getHpConsume() < _thisActor.getCurrentHp() && !_thisActor.isSkillDisabled(PhysicalSpecial1.getId()))
				{
					addUseSkillDesire(attacker, PhysicalSpecial1, 0, 0, 1000000);
