 ...
 * @date: 24.09.11 21:12
 */
public class CorpseDragonBeast extends Citizen
{
	public int DESPAWN_TIME = 5000;
	private GCSArray<Long> attackers;

	public CorpseDragonBeast(L2Character actor)
	{
		super(actor);
	...
	public boolean onTalk(L2Player talker)
	{
		...
		if(talker.isQuestStarted(456) && talker.getQuestState(456).getMemoState() == 1)
		{
			...
			{
				if(talker.getItemCountByItemId(17253) >= 1)
				{
					_thisActor.showPage(talker, "corpse_dragon_beast_q0456_03.htm", 456);
				}
				else
				{
					_thisActor.showPage(talker, "corpse_dragon_beast001.htm");
				}
			}
			else if(!attackers.contains(talker.getStoredId()))
			{
				_thisActor.showPage(talker, "corpse_dragon_beast_q0456_02.htm", 456);
			}
			else if(i0 == 0)
			{
				_thisActor.showPage(talker, "corpse_dragon_beast_q0456_02.htm", 456);
		...
		if(talker.isQuestStarted(456) && talker.getQuestState(456).getMemoState() == 1)
		{
			...
			if(attackers.contains(talker.getStoredId()) && i0 == _thisActor.param3 && i0 > 0)
			{
				if(ask == 456)
				{
					if(reply == 1)
					{
						if(talker.getItemCountByItemId(17251) == 0 || talker.getItemCountByItemId(17252) == 0)
						{
							talker.addItem("Quest", 17253, 1, _thisActor, true);
							_thisActor.showPage(talker, "corpse_dragon_beast_q0456_01.htm", 456);
							attackers.remove(talker.getStoredId());
						}
						if(talker.getItemCountByItemId(17251) == 1 && talker.getItemCountByItemId(17252) == 1)
						{
