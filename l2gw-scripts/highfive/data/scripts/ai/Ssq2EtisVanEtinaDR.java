 ...
 * @date: 06.10.11 19:34
 */
public class Ssq2EtisVanEtinaDR extends WarriorUseSkill
{
	public int p_TIMER_THINK = 500;
	public int p_TIMER_START_UP = 1000;
	public int p_TIMER_START_UP_GAP = 1000;

	public Ssq2EtisVanEtinaDR(L2Character actor)
	{
		super(actor);
		IsAggressive = 1;
	...
	{
		super.onEvtSpawn();
		//ServerVariables.set("GM_" + 80406, _thisActor.id);
	...
	protected void onEvtTimer(int timerId, Object arg1, Object arg2)
	{
		...
		else if(timerId == p_TIMER_THINK)
		{
			...
			if(c0 != null && c0.isPlayer())
			{
				...
				{
					if(SkillTable.getInstance().getInfo(439549953).getMpConsume() < _thisActor.getCurrentMp() && SkillTable.getInstance().getInfo(439549953).getHpConsume() < _thisActor.getCurrentHp() && !_thisActor.isSkillDisabled(SkillTable.getInstance().getInfo(439549953).getId()))
					{
						addUseSkillDesire(c0, 439549953, 0, 1, 1000000);
					}
				}
			}
	...
	{
		_thisActor.notifyAiEvent(_thisActor.getLeader(), CtrlEvent.EVT_SCRIPT_EVENT, 90210, 0, null);
		//int i0 = ServerVariables.getInt("GM_" + 80405);
		L2NpcInstance c0 = InstanceManager.getInstance().getNpcById(_thisActor, 18951);
		if(c0 != null)
		{
			_thisActor.notifyAiEvent(c0, CtrlEvent.EVT_SCRIPT_EVENT, 90210, 0, null);
		}
